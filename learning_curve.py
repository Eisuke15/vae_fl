...
from common import (central_save_net_path, device, fl_save_net_path,
                    mkdir_if_not_exists, mnist_data_root, transform)
from net import VAE
...
parser.add_argument('--fl', action="store_true", help="Plot the learning curve for federated learning.")
...
if args.fl:
    ...
    image_path = mkdir_if_not_exists('./learning_curve/fl/')
    ...
    for epoch in trange(n_epoch):
        ...
        for images, _ in test_dataloader:
            ...
            loss = F.binary_cross_entropy(y, images, reduction="sum") / images.size()[0]
            train_loss.append(loss.item())
            losses[epoch] = np.mean(train_loss)
    ...
    image_path = mkdir_if_not_exists('./learning_curve/central/')
    plt.savefig(os.path.join(image_path, f'nz{args.nz:02d}.png'), bbox_inches='tight')
